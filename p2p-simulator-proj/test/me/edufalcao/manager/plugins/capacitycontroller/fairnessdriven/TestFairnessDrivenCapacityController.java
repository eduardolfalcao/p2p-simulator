...
public class TestFairnessDrivenCapacityController {
	...
	@Test
	public void testGetMaxCapacityFromFairness() {
		fail("Not yet implemented");
	}

	@Test
	public void testUpdatePairwiseFairness() {
		...
		assertEquals(0, mockedFdController.getCurrentPairwiseFairness(), ACCEPTABLE_ERROR);
		mockedFdController.updatePairwiseFairness(peer2);
		assertEquals(-1, mockedFdController.getCurrentPairwiseFairness(), ACCEPTABLE_ERROR);
		...
		assertEquals(0, mockedFdController.getCurrentPairwiseFairness(), ACCEPTABLE_ERROR);
		mockedFdController.updatePairwiseFairness(peer2);
		assertEquals(0.5, mockedFdController.getCurrentPairwiseFairness(), ACCEPTABLE_ERROR);
		
	}

	@Test
	public void testUpdateGlobalFairness() {
		peer1.getRequests().add(new Request("id1", peer2, peer1, 10, 50));	//peer1 donates to peer2
		peer1.getRequests().add(new Request("id2", peer1, peer2, 20, 100));	//peer2 donates to peer1
		...
		mockedAccountingPlugin.updateAll();				
		assertEquals(0, mockedFdController.getCurrentGlobalFairness(), ACCEPTABLE_ERROR);
		mockedFdController.updateGlobalFairness();
		assertEquals(-1, mockedFdController.getCurrentGlobalFairness(), ACCEPTABLE_ERROR);
		...
		assertEquals(-1, mockedFdController.getCurrentGlobalFairness(), ACCEPTABLE_ERROR);
		mockedFdController.updateGlobalFairness();
		assertEquals(35.0/20.0, mockedFdController.getCurrentGlobalFairness(), ACCEPTABLE_ERROR);
		
		doReturn(120).when(mockedAccountingPlugin).getTime();
		mockedAccountingPlugin.updateAll();		
		assertEquals(35.0/20.0, mockedFdController.getCurrentGlobalFairness(), ACCEPTABLE_ERROR);
		mockedFdController.updateGlobalFairness();
		assertEquals(4, mockedFdController.getCurrentGlobalFairness(), ACCEPTABLE_ERROR);		
	}
